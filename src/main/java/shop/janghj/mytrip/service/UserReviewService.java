...
import shop.janghj.mytrip.repository.UserReviewLikeRepository;
import shop.janghj.mytrip.repository.UserReviewRepository;
import shop.janghj.mytrip.security.UserDetailsImpl;
import shop.janghj.mytrip.dto.UserReviewDto;
import shop.janghj.mytrip.util.S3Manager;
...
public class UserReviewService {
    ...
    @Transactional
    public UserReview putUserReview(Long reviewId, UserReviewDto userReviewDto, MultipartFile multipartFile, UserDetailsImpl nowUser) throws IOException {
        UserReview editReview = userReviewRepository.findById(reviewId).orElseThrow(
                () -> new NullPointerException("해당 리뷰가 존재하지 않습니다."));
        editReview.setTitle(userReviewDto.getTitle());
        editReview.setPlace(userReviewDto.getPlace());
        editReview.setReview(userReviewDto.getReview());
    ...
    public ResponseEntity<?> postUserReview(UserReviewDto userReviewDto, MultipartFile multipartFile, UserDetailsImpl nowUser) throws IOException {
        ...
        if (multipartFile == null) { // 처음 등록할 때 사진 선택하지 않으면 기본 이미지 저장
            userReview.setReviewImgUrl("https://d263rjzy8hqdjm.cloudfront.net/basic.jpg");
    ...
    public void deleteUserReview(Long reviewId, UserDetailsImpl nowUser) {
        ...
        if (!reviewImgUrl.equals("https://d263rjzy8hqdjm.cloudfront.net/basic.jpg")) { // 기본 이미지가 아닐 때만 S3에서 삭제
